...
Alteran needs Deno version management, but not at the cost of turning
`.runtime/` into a multi-version toolchain manager. It also needs to distinguish
between changing desired project configuration and upgrading installed tooling.
...
- `alteran upgrade --deno[=...]` upgrades the installed Alteran-managed Deno
  runtime without silently rewriting configuration policy

`alteran update` remains dependency-update semantics, not runtime policy or
Alteran self-upgrade policy.
