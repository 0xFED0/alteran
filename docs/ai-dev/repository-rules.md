...
It is not a higher authority than `docs/spec/` or `docs/adr/`. Its job is to make mandatory repository constraints hard to miss before editing.
...
- Keep managed execution explicit. Plain Deno stays plain unless the route goes through Alteran on purpose.
- Keep project context project-scoped. Do not implicitly reuse foreign `ALTERAN_*` runtime or logging context across projects.
- Keep cross-project execution explicit through Alteran-owned surfaces rather than hidden path tricks or ambient `PWD`.
- Keep canonical logs project-local under `.runtime/logs/`.
- Use archive sources for installation/materialization and run sources only for execution bootstrap.
- Keep support claims honest. GNU-based Linux is in scope; Alpine/musl is not a supported runtime target today.
...
- `setup` / `setup.bat` are public bootstrap surfaces and may be generated but intentionally tracked.
...
- If public behavior, naming, layout, config shape, tests, or publication contract changes, update `docs/spec/` first.
