...
`DENO_SOURCES` controls where Alteran looks for Deno release metadata and Deno
archives.
...
`ALTERAN_SRC` points Alteran at local authored source, which is especially
useful inside this repository and in the example projects.

## Legacy Compatibility

`ALTERAN_SOURCES` is treated as a legacy alias fallback for
`ALTERAN_RUN_SOURCES`.
...
- do not assume runnable sources alone are enough to materialize the local
  runtime
